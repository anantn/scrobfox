...
LFMRequest.prototype = {
    ...
    scrobble: function(song, key, cb) {
        ...
        this._params["timestamp"] = song.startedAt;
    ...
    nowPlaying: function(song, key, cb) {
        ...
        this._doPost(function(result) {
            if (typeof result != "object" || "error" in result) {
                cb(false);
                console.log("Error in nowPlaying: " + JSON.stringify(result));
            } else {
                cb(result);
            }
        });
...
function scrobble(song, key) {
    /* TODO: Handle failures and queue scrobbles */
    let sreq = new LFMRequest();
    sreq.scrobble(song, key, function(res) {
        //console.log("Done scrobble: " + JSON.stringify(res));
    });
}

function nowPlaying(song, key) {
    let nreq = new LFMRequest();
    nreq.nowPlaying(song, key, function(res) {
        //console.log("Done nowPlaying: " + JSON.stringify(res));
    });
}
